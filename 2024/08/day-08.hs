...
-- solve = length . nub . (\x -> concatMap (allAntinodes (length x, length (head x)) . (`antennas` x)) ['0'..'z']) . lines

-- signals :: (Eq a, Num n, Enum n) => ((Int, Int) -> [(n, Int)] -> [a]) -> [[Char]] -> [a]
signals :: (Eq a) => (Point -> [Point] -> [a]) -> [String] -> [a]
signals fn = nub . (\x -> concatMap (fn (length x, length (head x)) . (`antennas` x)) ['0'..'z'])
...
antinodes max xs = concatMap (\a -> filter (valid max) (map (antinode . (a,)) (filter (/= a) xs))) xs
